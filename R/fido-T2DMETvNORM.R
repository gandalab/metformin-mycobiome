...
### get only taxa that are shared between the four papers
shared <- psf %>% 
  ps_filter(paper %in% c("elbere2020", "qin2012", "karlsson2013")) %>% 
...
psh <- psh %>% 
  ...
  ps_filter(phenmet %in% c("T2D_metformin", "healthy_no.metformin")) %>% 
  # 1/23; dummy variables
  ps_mutate(metformin = if_else(phenmet == "T2D_metformin", 1, 0)) %>% 
  # get only T2D-MET v NORM papers
  ps_filter(paper %in% c("elbere2020", "qin2012", "karlsson2013"))
# 344 samples and 40 taxa

### NO paired samples

metadata <- sample_data(psh)
meta <- data.frame(metadata) %>% 
  select(metformin, paper)
...
## save parameters
pars <- params$Lambda
save(pars, file = "results/T2DMETvNORM-params.RData")
...
all_par <- rbind(qin %>% mutate(Study = "Qin 2012"), 
                 elb %>% mutate(Study = "Elbere2020"), 
                 karl %>% mutate(Study = "Karlsson 2013"))

save(all_par, file = "results/T2DMETvNORM-bypaper-allparams.RData")
