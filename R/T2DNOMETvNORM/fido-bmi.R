...
# how many papers have BMI metrics?
psf %>% samdat_tbl() %>% 
  group_by(paper) %>% 
  get_summary_stats(bmi.kg.m2, type = "mean_sd")
# forslund, karlsson, lechatlier, li, qin, sun, elbere2020, nielsen

### get only taxa that are shared between all papers
shared <- psf %>% ps_filter(!paper %in% c("wu2017")) %>% merge_samples(group = "paper") %>% 
  tax_transform("pa") %>% 
  tax_filter(min_prevalence = 8)
...
df <- data.frame(metadata) %>% 
  ...
  mutate(bmi.stand = (bmi.kg.m2 - mean(bmi.kg.m2) ) / sd(bmi.kg.m2)) %>% 
  mutate(metformin = if_else(metformin == "metformin", 1, 0))

# get summary stats
df %>% get_summary_stats(bmi.kg.m2, type = "common")

# get otu tabl
otu <- otu_table(ps)

## make model matrix
X <- t(model.matrix(~ bmi.stand + metformin + paper, data = df))
...
sig.params <- params$Lambda %>% 
  ...
  filter(sign(p2.5) == sign(p97.5)) # 5

# save results
bmipar <- params
save(bmipar, file = "results/fido-bmi-params.RData")
...
df <- data.frame(metadata) %>% 
  ...
  # standardize fbg
  mutate(bmi.stand = (bmi.kg.m2 - mean(bmi.kg.m2) ) / sd(bmi.kg.m2)) %>% 
  dplyr::select(metformin, bmi.stand)
...
df <- data.frame(metadata) %>% 
  ...
  # standardize fbg
  mutate(bmi.stand = (bmi.kg.m2 - mean(bmi.kg.m2) ) / sd(bmi.kg.m2)) %>% 
  dplyr::select(bmi.stand, metformin)
...
## NIELSEN 2014
ps <- psf %>% 
  ps_filter(paper == "nielsen2014") 

df <- data.frame(metadata) %>% 
  filter(paper == "nielsen2014") %>% 
  drop_na(bmi.kg.m2) %>% 
  # standardize fbg
  mutate(bmi.stand = (bmi.kg.m2 - mean(bmi.kg.m2) ) / sd(bmi.kg.m2)) %>% 
  dplyr::select(bmi.stand)

ni <- myFIDO(ps, df, get_r2 = FALSE)
...
all_pars <- rbind(fors %>% mutate(Study = "Forslund 2014"),
                  ...
                  ni %>% mutate(Study = "Nielsen 2014"))

save(all_pars, file = "results/fido-bmi-bypaper-params.RData")
...
### THRESHOLD: there were 8 studies in the comparison,
...
mod1 <- outdf %>% 
  filter(signs == TRUE & size == TRUE) # 2 of which Ustilago remains if threshold is set to 6
