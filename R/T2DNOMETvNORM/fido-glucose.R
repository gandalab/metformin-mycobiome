...
# how many papers have glucose metrics?
psf %>% samdat_tbl() %>% 
  group_by(paper) %>% 
  get_summary_stats(fasting.glucose.mg.dL, type = "mean_sd")
...
df <- data.frame(metadata) %>% 
  ...
  mutate(fbg.stand = (fasting.glucose.mg.dL - mean(fasting.glucose.mg.dL) ) / sd(fasting.glucose.mg.dL)) %>% 
  mutate(metformin = if_else(metformin == "metformin", 1, 0))

## get summary stats for results
df %>% get_summary_stats(fasting.glucose.mg.dL, type = "common")

# get otu tabl
otu <- otu_table(ps)

## make model matrix
X <- t(model.matrix(~ fbg.stand + metformin + paper, data = df))
...
sig.params <- params$Lambda %>% 
  ...
  filter(sign(p2.5) == sign(p97.5)) # 7

# save results
glucpar <- params
save(glucpar, file = "results/fido-glucose-params.RData")
...
df <- data.frame(metadata) %>% 
  ...
  # standardize fbg
  mutate(fbg.stand = (fasting.glucose.mg.dL - mean(fasting.glucose.mg.dL) ) / sd(fasting.glucose.mg.dL)) %>% 
  dplyr::select(metformin, fbg.stand)
...
df <- data.frame(metadata) %>% 
  ...
  # standardize fbg
  mutate(fbg.stand = (fasting.glucose.mg.dL - mean(fasting.glucose.mg.dL) ) / sd(fasting.glucose.mg.dL)) %>% 
  dplyr::select(fbg.stand, metformin)
...
save(all_pars, file = "results/fido-glucose-bypaper-params.RData")

## ---- FILTER ----


## the significant coords from global comparison
sigs <- sig.params$coord # 7
