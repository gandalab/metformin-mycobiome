### FIDO; short-term RCT T2D-MET vs T2D-NOMET
# EVS 2/2023
...
## 187 samples and 36 taxa; 22 Wu, 22 Sun, 50 Elbere
...
psf %>% 
  ...
  drop_na(hba1c.perc) %>% 
  group_by(metformin) %>% 
  get_summary_stats(hba1c.perc, type = "common")
...
## save parameters
pars <- params$Lambda
save(pars, file = "results/T2DMETvNOMET_RCT-params.RData")
...
save(all_par, file = "results/T2DMETvT2DNOMET_RCT-bypaper-allparams.RData")
